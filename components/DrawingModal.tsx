...
import { X, ChevronRight, Save, Square, Triangle, Circle, Undo, Eraser, PenTool, Spline, PaintBucket, Minus, Plus, Eye, EyeOff } from 'lucide-react';
...
type ToolType = 'pen' | 'bezier' | 'square' | 'triangle' | 'circle' | 'bucket';
...
export const DrawingModal: React.FC<DrawingModalProps> = ({ 
  ...
  const [showGuide, setShowGuide] = useState<boolean>(true); // Estado para o guia de fundo
  ...
  useEffect(() => {
    ...
    if (activeTool !== 'pen' && activeTool !== 'bucket' && startPoint && currentPoint) {
        ...
        <div className="flex-1 bg-slate-950 relative overflow-hidden flex items-center justify-center touch-none">
          <canvas
            ...
            className={`bg-slate-950 touch-none border border-slate-800 shadow-inner w-full h-auto max-w-[500px] max-h-[500px]
                ${activeTool === 'bucket' ? 'cursor-alias' : 'cursor-crosshair'}
            `}
